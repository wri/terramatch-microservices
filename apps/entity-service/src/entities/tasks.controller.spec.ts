...
describe("TasksController", () => {
  ...
  beforeEach(async () => {
    ...
    const module = await Test.createTestingModule({
      ...
      providers: [PolicyService, { provide: TasksService, useValue: (service = createMock<TasksService>()) }]
    }).compile();
  ...
  describe("taskIndex", () => {
    ...
    it("should add DTOs to the document", async () => {
      ...
      expect(authSpy).toHaveBeenCalledWith("read", tasks);
  ...
  describe("taskGet", () => {
    it("should return the full dto", async () => {
      ...
      expect(authSpy).toHaveBeenCalledWith("read", task);
