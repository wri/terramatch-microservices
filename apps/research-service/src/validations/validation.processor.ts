...
export interface ProjectValidationJobData {
  projectId: number;
  validationTypes: ValidationType[];
  delayedJobId: number;
}
...
export class ValidationProcessor extends DelayedJobWorker<ValidationJobData> {
  ...
  private async processSiteValidation(job: Job<SiteValidationJobData>) {
  ...
  private async processProjectValidation(job: Job<ProjectValidationJobData>) {
    const { projectId, validationTypes } = job.data;

    const polygonUuids = await this.validationService.getProjectPolygonUuidsForValidation(projectId);
    if (polygonUuids.length === 0) {
      throw new DelayedJobException(404, `No polygons found for project ${projectId}`);
    }
