...
describe('ResetPasswordService', () => {
  ...
  it('should send a reset password email to the user', async () => {
    const user = await UserFactory.create();
    const localizationBody = await LocalizationKeyFactory.create({ key: 'reset-password.body', value: 'Reset your password by clicking on the following link: link' });
    const localizationSubject = await LocalizationKeyFactory.create({ key: 'reset-password.subject', value: 'Reset Password' });
    jest.spyOn(User, 'findOne').mockImplementation(() => Promise.resolve(user));
  ...
  it('should an error when invalid token', async () => {
    ...
    const token = 'fake token'
    await expect(service.resetPassword(token, newPassword)).rejects.toThrow(new BadRequestException('Provided token is invalid or expired'));
  });
