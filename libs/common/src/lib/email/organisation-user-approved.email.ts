...
export type OrganisationUserApprovedEmailData = {
  organisationId: number;
  userId: number;
};

export class OrganisationUserApprovedEmail extends EmailSender<OrganisationUserApprovedEmailData> {
  static readonly NAME = "organisationUserApproved";

  private readonly logger = new TMLogger(OrganisationUserApprovedEmail.name);

  constructor(data: OrganisationUserApprovedEmailData) {
    super(OrganisationUserApprovedEmail.NAME, data);
  }
  ...
  async send(emailService: EmailService) {
    ...
    await emailService.sendI18nTemplateEmail(
      ...
      user.locale,
      {
        subject: "organisation-user-approved.subject",
        title: "organisation-user-approved.title",
        body: "organisation-user-approved.body"
      },
      {
        i18nReplacements: {
    ...
    this.logger.log(
      `Sent organisation user approved email to user ${this.data.userId} for organisation ${this.data.organisationId}`
    );
  }
}
