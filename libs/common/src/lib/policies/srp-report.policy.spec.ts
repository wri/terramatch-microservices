...
describe("SrpReportPolicy", () => {
  ...
  it("allows managing disturbance reports for own projects", async () => {
    ...
    const pr1 = await SrpReportFactory.create({ projectId: p1.id });
    const pr2 = await SrpReportFactory.create({ projectId: p2.id });
    const pr3 = await SrpReportFactory.create({ projectId: p3.id });
    const pr4 = await SrpReportFactory.create({ projectId: p4.id });
