...
export class User extends BaseEntity {
  ...
  async primaryOrganisation() {
    if (this._primaryOrganisation == null) {
      ...
      this._primaryOrganisation = (await this.organisationsRequested().getOne()) ?? false;
    }
  ...
  async frameworks(): Promise<{ name: string, slug: string }[]> {
    ...
    const isAdmin = (await this.roles()).find(role => role.startsWith('admin-')) != null;
  ...
  async organisationUserStatus(): Promise<string | undefined> {
    ...
    return (await OrganisationUser
      .findOne({ where: { userId: this.id, organisationId: org.id }}))
      ?.status;
  }
}
